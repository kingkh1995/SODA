 ...
 * 作为通道常量载体与 {@code Verification} create 构造<b>必传</b>输入（决定码长、过期时间与字符集，
 * 效果物化进 {@code VerificationCode}，不落验证实体，见 ADR-0011）。
 * 码形是通道级规则（短信=纯数字、邮箱=字母数字），非账号配置字段——账户实体不持有 policy
 * （2026-08-09 移除账户字段，见 ADR-0018）。2026-08-16（见 ADR-0026）策略决策属调用方<b>场景</b>：
 * 聚合不做通道→策略映射；通道默认（{@link #DEFAULT_SMS}/{@link #DEFAULT_EMAIL}）仍是通道级规则
 * 常量，但按场景方法选择的专属策略可覆盖（UCC 专属策略：双通道统一纯数字/短时效，不引用通道默认）。
 * <p>
