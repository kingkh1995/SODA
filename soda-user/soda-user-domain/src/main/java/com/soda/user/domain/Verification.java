...
/**
 * 验证聚合根 — 验证码生命周期载体，独立于 User（URG 场景发码时用户尚不存在，
 * 生命周期亦与 User 不一致，见 ADR-0021/0025）。
 * <p>
 * <b>双概念模型</b>（见 ADR-0026）：只感知 {@code source}（{@link VerificationSource}
 * ——<b>不透明</b>，scene/subject 纯字符串，非空即足，零行为耦合）与 {@code recipient}
 * （{@link VerificationRecipient}——多态密封，channel + 类型化地址，<b>通道判别栖身于此</b>）。
 * 领域内无任何 channel 行为——{@code verify}/{@code use}/{@code isExpiredAt} 全通道同构；
 * 发送验证码的主体是用户（UCC/ULG/UPR；URG 无用户例外），Verification 是协助方聚合，
 * 「UserVerification」是用例概念（非类，见 ADR-0026）。
 * <p>
 * 策略（{@link VerificationCodePolicy}）是创建时的<b>必传</b>输入（默认值由调用方工厂按
 * <b>场景方法</b>选择——UCC 专属策略：纯数字、短时效、双通道统一，见 ADR-0026），
 * 效果物化进 {@link VerificationCode}，不作为聚合属性持久化；send 投递是投递侧职责
 * （监听器按 recipient 类型匹配 sender）。
 * <p>
 * 状态跃迁：INITIALIZED → PENDING（{@link #markSent()}，投递成功由监听器调用）→ VERIFIED
 * → USED，或任一步过期。VERIFIED 为内存瞬态（消费流 verify→use 同事务完成，永不落库，
 * 见 {@code CredentialChangeDomainService}）。可变命令风格（与 {@link AuthAccount} 一致）。
 * <p>
...
public final class Verification extends Aggregate<Uuid> {
     ...
     * 缺字段在 Jackson 边界拒绝（docs/conventions/framework-type-contracts.md「JSON 序列化契约」+ ADR-0015 双屏障）。
     */
    ...
    /**
     * 创建验证聚合（创建路径，ID 创建时生成）。
     * <p>
     * {@code policy} 为<b>必传</b>参数（聚合不做通道→策略映射——策略决策属调用方<b>场景</b>，
     * 默认值由 {@code UserVerificationFactory} 按场景方法选择，见 ADR-0026），效果物化进
     * {@link VerificationCode}，不落验证实体。{@code source} 恒非空：UCC/ULG/UPR 的
     * subject = userId 裸键串（UCC 会话注入、ULG/UPR 端点反查），URG = 端点值串（见 ADR-0026）。
     * 注册 {@link VerificationCreatedEvent}——由 ApplicationService 持久化后发布，投递侧
     * 监听器事务提交后消费（见 ADR-0011）。
     */
