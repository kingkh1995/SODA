 ...
 * <p>
 * 在 {@link SmsVerification}/{@link EmailVerification} 的 create 工厂内注册（实体创建即需投递验证码）。
 * 由 ApplicationService 在持久化后经 {@link com.soda.component.domain.DomainEventBus} 发布；
 * 投递侧监听器（AFTER_COMMIT，见 ADR-0011）收到后执行 {@code send(sender)} 并落库 PENDING——
 * 记录先于发送持久化，且 DB 事务不跨外部投递通道持有。
 * <p>
