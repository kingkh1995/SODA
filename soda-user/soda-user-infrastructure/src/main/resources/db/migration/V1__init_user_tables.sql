...
-- 2026-08-15 变更记录：AbstractAuditable 曾当日强制 @Version（统一契约）后撤销——Verification 领域不带版本令牌，PO 层版本对跨加载竞态空转（见 ADR-0024 修订），@Version 仅保留于 UserPO（user 表 version 列，领域令牌承重）；user_verification 无 version 列。H2 MODE=MySQL 需 DATABASE_TO_LOWER=TRUE（V1 反引号小写表名）。
...
CREATE TABLE `user` (
    ...
    `state`         VARCHAR(1)   NOT NULL DEFAULT 'E' COMMENT '用户状态（E/D/R，ADR-0017 终态 R 可持久化）',
    `password_hash` VARCHAR(200) NOT NULL COMMENT '密码哈希（PasswordAuthAccount 落此列，ADR-0004 单表化修订）',
    `sms_login_enabled`  BOOLEAN NOT NULL DEFAULT TRUE COMMENT '手机号登录开关（SmsAuthAccount.active；0=关闭手机号登录，手机号仍为账号标识，支付宝/阿里云模式）',
    `email_login_enabled` BOOLEAN NOT NULL DEFAULT TRUE COMMENT '邮箱登录开关（EmailAuthAccount.active；0=关闭邮箱登录，邮箱仍为账号标识）',
    `version`       INT          NOT NULL DEFAULT 0 COMMENT '乐观锁版本号（JPA @Version 自动校验递增）',
...
CREATE TABLE `verification` (
    ...
    `active_key`  VARCHAR(160) NULL COMMENT '活跃键（source.compositeKey() = scene:subject；I/P 且未过期占槽，V/U 迁移清 NULL、过期 I/P 行 DELETE 释放；uk_active_key 硬保证单活跃）',
    `created_date`      DATETIME NOT NULL COMMENT '创建时间（审计，Spring Data auditing 维护，UTC 字面值；无 DB 默认值——CURRENT_TIMESTAMP 按会话时区生成会漂移，由应用恒填充）',
    `last_modified_date` DATETIME NOT NULL COMMENT '最后更新时间（审计，Spring Data auditing 维护，UTC 字面值；无 DB 默认值/ON UPDATE——避免会话时区漂移）',
    PRIMARY KEY (`id`),
