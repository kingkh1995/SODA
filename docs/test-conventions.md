...
| 命名      | 方法 `should_<行为>`（条件不足以消歧时补 `_when_<条件>`）；`@DisplayName` 中文说明                                                                                                                                                                                                       |
| 分组      | `@Nested` 按行为分组；方法数 ≤6 平铺                                                                                                                                                                                                                                                     |
| 数据      | 内联字面值；同一值 ≥3 个方法复用才提取类内常量                                                                                                                                                                                                                                           |
| 参数化    | `@ParameterizedTest` 仅 ≥4 组同构数据（`@NullAndEmptySource` 单独使用＝2 组，豁免该基线；与其它 Source 组合按合并组数计，≥3 组不豁免）；不足 4 组的同族同构负例允许内联——同一场景的 ≤3 个反例合并在一个方法内串行断言（示例：`AvatarTest` 相对路径 3 例）；≥10 组用 `@MethodSource` 分离 |
| 断言密度  | 每个方法一个逻辑断言（一个场景）；同族同构负例合并（上行）视作一个场景                                                                                                                                                                                                                   |
