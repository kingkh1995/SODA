...
2. **外键违反架构约束**：阿里《Java 开发手册》【强制】不得使用外键与级联；「完整性由应用层聚合边界保证」（如 `user_verification.user_id` 由 Verification 构造器按 scene 归一化约束，见 ADR-0021）与 DB 外键语义冲突。
...
- ADR-0021（user_verification.user_id 可空与构造器归一化）；soda-user-start `application.yml`（H2 数据源配置）
