...
| 方案 | 否定原因 |
|---|---|
| 枚举（DIGITS/LETTERS/ALPHANUMERIC，闭集） | 无法表达任意字符集；类别型惯例（ADR-0005）不适用于值型概念 |
