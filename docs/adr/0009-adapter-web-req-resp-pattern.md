...
description: 写 Web 层 Controller 与请求响应转换时读——入参出参用 XxxRequest/XxxResponse，MapStruct WebAssembler 双向转换，响应统一 Result 信封。
...
MapStruct 接口 `XxxWebAssembler` 做 Request→Command、DTO→Response 双向转换，响应统一包 `Result<T>` 信封
`{code, msg, data?, error?}`（定义于组件 web 通道，全模块共用）。两者变化频率不同（协议形状随前端变，Command/DTO
