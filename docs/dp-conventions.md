...
public record XxxId(@JsonValue T value) implements Identifier<T>, Comparable<XxxId> {
    ...
    public static XxxId valueOf(Object value) {
        if (value instanceof XxxId x) {                         // 已是对应类型时直接返回
            return x;
        }
...
public record Xxx(@JsonValue String value) implements Type, Comparable<Xxx> {
    ...
    public static Xxx valueOf(Object value) {
        if (value instanceof Xxx x) {                           // 已是对应类型时直接返回
            return x;
        }
...
public final class Xxx implements Type, Comparable<Xxx> {
    ...
    public static Xxx valueOf(Object value) {
        if (value instanceof Xxx x) {
            return x;
        }
        return new Xxx(ParseUtils.parseString(value));
    }
...
public final class Xxx implements Type, Comparable<Xxx> {
    ...
    /** 校验在构造器中完成 — 所有创建路径（{@code of}、{@code valueOf}、Jackson）最终都经过此构造器。 */
...
以下代码模式**禁止**出现在任何 DP 的构造器、`valueOf` 方法或任何校验上下文中：
...
4. 添加单一 `valueOf(Object)` 工厂方法，委托 `ParseUtils.parseXxx(value)` 做类型转换（仅单字段 DP 适用；多字段 DP 有多个 {@code @JsonValue} 字段时无需此步骤，通过构造器参数直接传入）
...
5. 添加 `valueOf(Object)` 工厂方法，委托 `ParseUtils.parseString(value)` → 构造器
6. `toString()` 手写为方括号格式
...
6. `valueOf(Object)` 委托到 `of(ParseUtils.parseXxx(value))` 而非直接 `new Xxx(…)`
7. `toString()` 手写为方括号格式
